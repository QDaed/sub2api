...
export const paymentAPI = {
  ...
  getMyOrders(params?: { page?: number; page_size?: number; status?: string }) {
    return apiClient.get<BasePaginationResponse<PaymentOrder>>(
      "/payment/orders/my",
      { params },
    );
  },
  ...
  verifyOrder(outTradeNo: string) {
    return apiClient.post<PaymentOrder>("/payment/orders/verify", {
  ...
  resolveOrderPublicByResumeToken(resumeToken: string) {
    return apiClient.post<PaymentOrder>("/payment/public/orders/resolve", {
      resume_token: resumeToken,
    });
  },

  /** Notify backend of Pay2S return-redirect payment result (no auth required) */
  pay2sReturn(params: Record<string, string>) {
    return apiClient.get("/payment/public/pay2s/return", { params });
  },
  ...
  getRefundEligibleProviders() {
    return apiClient.get<{ provider_instance_ids: string[] }>(
