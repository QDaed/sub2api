...
export function normalizeVisibleMethod(
  method: string,
): VisiblePaymentMethod | "" {
  const normalized =
    VISIBLE_METHOD_ALIASES[
      method.trim() as keyof typeof VISIBLE_METHOD_ALIASES
    ];
  return normalized ?? "";
}
...
export function buildCreateOrderPayload(
  ...
  const visibleMethod =
    normalizeVisibleMethod(input.paymentType) || input.paymentType.trim();
...
export function decidePaymentLaunch(
  ...
  const visibleMethod =
    normalizeVisibleMethod(context.visibleMethod) || context.visibleMethod;
  ...
  if (visibleMethod === "pay2s" && baseState.payUrl) {
    const paymentState = {
      ...baseState,
      payUrl: context.pay2sRouteUrl || baseState.payUrl || "",
    };
    return { kind: "pay2s_route", paymentState, recovery: paymentState };
  }
  ...
  if (baseState.clientSecret) {
    ...
    const kind: PaymentLaunchKind =
      stripeMethod === "alipay" && !context.isMobile
        ? "stripe_popup"
        : "stripe_route";
    const payUrl =
      kind === "stripe_popup"
