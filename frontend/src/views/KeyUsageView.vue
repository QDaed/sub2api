  ...
  <div class="relative flex min-h-screen flex-col bg-gray-50 dark:bg-dark-950">
    ...
    <main class="flex-1 w-full max-w-5xl mx-auto px-6 py-12">
      ...
      <div class="text-center mb-12">
        <h1
          class="text-3xl sm:text-4xl font-bold tracking-tight mb-3 text-gray-900 dark:text-white"
        >
          {{ t("keyUsage.title") }}
        </h1>
      ...
      <div class="max-w-xl mx-auto mb-14">
        <div class="flex gap-3">
          <div class="flex-1 relative">
            <div
              class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-dark-500"
            >
              <svg
                class="w-5 h-5"
                viewBox="0 0 24 24"
      ...
      <div v-if="showResults">
        ...
        <div v-else-if="resultData" class="space-y-6">
          ...
          >
            <div
              class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-gray-200 bg-white/90 shadow-sm backdrop-blur-sm dark:border-dark-700 dark:bg-dark-900/90"
            >
              <span
              ...
              <div class="flex justify-center">
                <div class="relative">
                  ...
                  >
                    ...
                    <template v-else>
                      <span
                        class="text-3xl font-bold tabular-nums text-gray-900 dark:text-white"
                      >
                        {{ displayPcts[i] ?? 0 }}%
                      </span>
                      <span
                        class="text-xs text-gray-500 dark:text-dark-400 mt-0.5"
                        >{{ t("keyUsage.used") }}</span
                      >
                      <span
                      ...
                      <p
                        ...
                        class="text-xs text-gray-400 dark:text-gray-500 mt-0.5 tabular-nums"
                      >
          ...
          >
            <div
              class="px-8 py-5 border-b border-gray-200 dark:border-dark-700"
            >
              <h3
                class="text-sm font-semibold uppercase tracking-wider text-gray-500 dark:text-dark-400"
              >
          ...
          >
            ...
            <div
              class="grid grid-cols-2 md:grid-cols-4 gap-px bg-gray-100 dark:bg-dark-800"
            >
              <div
              ...
              >
                <div class="text-xs text-gray-500 dark:text-dark-400 mb-1">
                  {{ cell.label }}
                </div>
                <div
          ...
          >
            ...
            <div class="overflow-x-auto">
              <table class="w-full">
                <thead>
                  <tr
                    class="border-b border-gray-200 bg-gray-50 dark:border-dark-700 dark:bg-dark-950"
                  >
                    <th
                      class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-dark-400"
                    >
                      {{ t("keyUsage.model") }}
                    </th>
                    <th
                      class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-dark-400"
                    >
                      {{ t("keyUsage.requests") }}
                    </th>
                    <th
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td
                      class="px-4 py-3 text-sm tabular-nums text-right text-gray-700 dark:text-dark-200"
                    >
                      {{ fmtNum(m.requests) }}
                    </td>
                    <td
                    ...
                    <td
                      class="px-4 py-3 text-sm tabular-nums text-right font-medium text-gray-900 dark:text-white"
                    >
  ...
  if (data.mode === "quota_limited") {
    ...
    if (data.rate_limits) {
      const windowMap: Record<string, string> = {
        "5h": "5H",
        "1d": shortQuotaWindowLabel("1d"),
        "7d": shortQuotaWindowLabel("7d"),
      };
      for (const rl of data.rate_limits) {
    ...
    if (data.subscription) {
      ...
      if (sub.daily_limit_usd > 0) {
        ...
        rows.push({
          ...
          iconColor: "text-primary-500",
          iconSvg: ICON_DOLLAR,
          label: `${t("keyUsage.usedQuota")} (${t("keyUsage.dayShort")})`,
      ...
      if (sub.weekly_limit_usd > 0) {
        ...
        rows.push({
          ...
          iconSvg: ICON_DOLLAR,
          label: `${t("keyUsage.usedQuota")} (${t("keyUsage.weekShort")})`,
      ...
      if (sub.monthly_limit_usd > 0) {
        ...
        rows.push({
          ...
          iconColor: "text-emerald-500",
          iconSvg: ICON_DOLLAR,
          label: `${t("keyUsage.usedQuota")} (${t("keyUsage.monthShort")})`,
...
function shortQuotaWindowLabel(window: string): string {
  switch (window) {
    case "1d":
      return t("keyUsage.dayShort");
    case "7d":
      return "7D";
    case "week":
      return t("keyUsage.weekShort");
    case "month":
      return t("keyUsage.monthShort");
    default:
      return window;
  }
}
...
function formatDate(iso: string | null | undefined): string {
  ...
  return d.toLocaleDateString(resolveBrowserLocale(locale.value), {
    year: "numeric",
    month: "long",
    day: "numeric",
  });
