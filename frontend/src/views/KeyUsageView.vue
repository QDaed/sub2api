...
const detailRows = computed<DetailRow[]>(() => {
  ...
  if (data.mode === 'quota_limited') {
    ...
    if (data.rate_limits) {
      const windowMap: Record<string, string> = { '5h': '5H', '1d': shortQuotaWindowLabel('1d'), '7d': shortQuotaWindowLabel('7d') }
      for (const rl of data.rate_limits) {
    ...
    if (data.subscription) {
      ...
      if (sub.daily_limit_usd > 0) {
        ...
        rows.push({
          iconBg: 'bg-primary-500/10', iconColor: 'text-primary-500', iconSvg: ICON_DOLLAR,
          label: `${t('keyUsage.usedQuota')} (${t('keyUsage.dayShort')})`, value: `${usd(sub.daily_usage_usd)} / ${usd(sub.daily_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.weekly_limit_usd > 0) {
        ...
        rows.push({
          ...
          label: `${t('keyUsage.usedQuota')} (${t('keyUsage.weekShort')})`, value: `${usd(sub.weekly_usage_usd)} / ${usd(sub.weekly_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.monthly_limit_usd > 0) {
        ...
        rows.push({
          ...
          label: `${t('keyUsage.usedQuota')} (${t('keyUsage.monthShort')})`, value: `${usd(sub.monthly_usage_usd)} / ${usd(sub.monthly_limit_usd)}`, valueClass: getUsageColor(pct),
        })
      }
      if (sub.expires_at) {
        rows.push({
...
function formatDate(iso: string | null | undefined): string {
  ...
  return d.toLocaleDateString(resolveBrowserLocale(locale.value), { year: 'numeric', month: 'long', day: 'numeric' })
}
