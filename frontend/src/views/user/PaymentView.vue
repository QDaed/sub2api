        ...
        <template v-else>
            ...
            <template v-else>
              ...
              <div v-if="validAmount > 0" class="card p-6">
                <div class="space-y-2 text-sm">
                  ...
                  <div
                    v-if="isPay2SSelected && pay2sVndAmountTopup > 0"
                    class="flex justify-between"
                  >
                    <span class="text-gray-500 dark:text-gray-400">
                      {{ t("payment.pay2sVndEquivalentTopup") }}
                    </span>
                    <span class="font-medium text-gray-900 dark:text-white">
                      {{
                        new Intl.NumberFormat(
                          (localeCode as string) || undefined,
                        ).format(pay2sVndAmountTopup)
                      }}
                      VND
                    </span>
                  </div>
                  <div
                    v-if="isPay2SSelected"
                    class="text-xs text-gray-400 dark:text-gray-500"
                  >
                    {{ t("payment.pay2sExchangeRate") }}
                  </div>
                  <div
          ...
          <template v-else-if="activeTab === 'subscription'">
            ...
            <template v-if="selectedPlan">
              ...
              <div v-if="isPay2SSelected" class="card p-6">
                <div class="space-y-2 text-sm">
                  <div
                    v-if="pay2sVndAmountSubscribe > 0"
                    class="flex justify-between"
                  >
                    <span class="text-gray-500 dark:text-gray-400">
                      {{ t("payment.pay2sVndEquivalentSubscribe") }}
                    </span>
                    <span class="font-medium text-gray-900 dark:text-white">
                      {{
                        new Intl.NumberFormat(
                          (localeCode as string) || undefined,
                        ).format(pay2sVndAmountSubscribe)
                      }}
                      VND
                    </span>
                  </div>
...
const isPay2SSelected = computed(
  () => normalizeVisibleMethod(selectedMethod.value) === "pay2s",
);
const pay2sVndAmountTopup = computed(() =>
  isPay2SSelected.value && totalAmount.value > 0
    ? Math.round(totalAmount.value * 27000)
    : 0,
);
const pay2sVndAmountSubscribe = computed(() =>
  isPay2SSelected.value && subTotalAmount.value > 0
    ? Math.round(subTotalAmount.value * 27000)
    : 0,
);
...
async function createOrder(
  ...
  try {
    ...
    if (decision.kind === "pay2s_route" && decision.paymentState.payUrl) {
      if (isMobileDevice()) {
        window.location.href = decision.paymentState.payUrl;
        return;
      }
