<template>
  <div
    class="flex min-h-screen items-center justify-center bg-gray-50 px-4 dark:bg-dark-900"
  >
    <div class="w-full max-w-md space-y-6">
      ...
      <template v-else>
        ...
        <div class="text-center">
          ...
          >
            <div
              class="h-10 w-10 animate-spin rounded-full border-4 border-yellow-500 border-t-transparent"
            ></div>
          </div>
          <div
            v-else
...
onMounted(async () => {
  ...
  const hasPay2SReturnContext =
    readRouteQueryString("m2signature").trim() !== "" &&
    readRouteQueryString("resultCode").trim() !== "";
  const shouldUsePublicOutTradeNo =
    outTradeNo !== "" &&
    (hasLegacyFallbackContext ||
      hasPay2SReturnContext ||
      routeOrderId > 0 ||
      orderId > 0);
  ...
  // Pay2S sends payment confirmation in the return URL redirect.
  // Forward the query params to the backend so it can verify the signature
  // and update the order status (webhooks may not always arrive).
  if (hasPay2SReturnContext && outTradeNo) {
    try {
      const pay2sParams: Record<string, string> = {};
      Object.entries(route.query).forEach(([key, value]) => {
        if (Array.isArray(value)) {
          pay2sParams[key] = String(value[0] || "");
        } else if (typeof value === "string") {
          pay2sParams[key] = value;
        }
      });
      await paymentAPI.pay2sReturn(pay2sParams);
      // Refresh order status after backend processing
      if (orderId) {
        const refreshed = await paymentStore.pollOrderStatus(orderId);
        if (refreshed) {
          setResolvedOrder(refreshed);
        }
      }
    } catch (_err: unknown) {
      // Non-fatal: backend verification may fail if signature formula differs.
      // The order will remain pending and the user can wait for webhook or retry.
    }
  }
